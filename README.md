# SWET

**A chronicle of Knox County, Kentucky — 14–31 August 2026.**

> *These are the end times. There was no hope of survival. This is how you died.*

That is what Project Zomboid tells you before you play a second of it. Over
eighteen days on a private server, three people supplied the details: thirteen
characters lost, a forge built from nothing, one man blown up by his own pipe
bomb, and the same category of roof fatal twice.

📖 **[Read it](https://senshuk.github.io/swet-chronicle/)**

Or read [`swet-chronicle.md`](swet-chronicle.md) here on GitHub.

---

## The record

| | stacey | senpai | frank |
|---|---|---|---|
| Deaths | **0** | 5 | 6 |
| Longest single life | **2,713 hrs** | 885 hrs | 719 hrs |
| Maxed skills | Cooking, Foraging, Butchering, Blacksmithing | Carpentry | Tailoring, Carving |

Seventeen chapters, four appendices and a map, assembled from nineteen days of
dedicated-server logs and the group's own chat. Every quoted line is verbatim.

The appendices are Tolkien's apparatus applied to a video game: a Tale of Years
running the canonical Knox Event chronology beside the group's own; the line of
succession by which one player's seven characters each inherited the last one's
skills out of a notebook; a note on the dialect, which inflects for *speaker
identity* rather than meaning; and a description of the two holdings.

The map is not drawn. It is 1,296 logged vehicle positions joined in order —
every death marked where the server recorded it.

The reader filters by survivor and by thread — deaths, roofs, bombs, building,
the journal, driving — and pages through whatever the filter leaves. Deaths are
marked down the timeline spine on the left, so the shape of the run is visible
before you read a word of it.

## Contents

- `index.html` — the chronicle reader. Self-contained; no build step.
- `swet-chronicle.md` — the same text as plain markdown.

## Notes

Source logs are deliberately excluded: they contain account identifiers.
